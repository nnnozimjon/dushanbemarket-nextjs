...
export default function CatalogPage() {
  ...
  const {
    ...
    ObjectToParams({ pageSize, pageNumber, order: "asc", sub_category_id: params.id })
  );
  ...
  return (
    <Container size={'xl'}>
      <div className="md:py-10">
        ...
        >
          <h1 className="text-[24px] md:text-[2em]">
            {category_name}
          </h1>
        ...
        <Flex className="py-[30px]" gap={'lg'}>
          ...
          <div className="w-full">
            <SimpleGrid
              ...
              verticalSpacing={{ base: 'md', sm: 'xl' }}
            >
              {isLoadingProducts && Array.from({ length: 12 }, (_, index) => (
        ...
        {dataProducts?.payload?.length == 0 && <Flex direction={'column'} gap={'lg'} className=' w-full' align={'center'} justify={'center'}>
          <Image src={empty.src} alt='empty' className='h-[120px] w-[120px] md:h-[180px] md:w-[180px] text-[#01B766]' />
          <Flex direction={'column'} align={'center'}>
            <Text className='text-[#212121] font-bold text-[16px] text-center'>
              К сожалению, в этой категории нет товаров.
            </Text>
            <Text className='text-center'>
              Пожалуйста, проверьте позже или воспользуйтесь поиском для поиска нужного продукта.
            </Text>
          </Flex>
        </Flex>}
      </div>
