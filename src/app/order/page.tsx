...
import { Alert, Box, Button, Container, Flex, InputBase, NumberInput, Paper, Text, Textarea } from '@mantine/core'
            ...
            <div className='my-4 md:py-10 flex lg:flex-row flex-col gap-5'>
                                ...
                                <div className='bg-[rgb(247,248,249)] w-full p-5 rounded-md'>
                                    <Paper radius={'lg'} className='p-5'>
                                        ...
                                        <Flex gap={'lg'} direction={'column'}>
                                            <InputBase value={order?.phone_number} withAsterisk label="Номер телефона"  placeholder='Номер телефона' classNames={{ input: 'h-[60px] rounded-[16px]' }} onChange={(e) => setOrder((prev) => ({
                                                ...prev,
                                                phone_number: formatPhoneNumber(e.target.value)
                                            }))} />
                                            <InputBase withAsterisk  label="Адрес доставки" placeholder='Адрес доставки' classNames={{ input: 'h-[60px] rounded-[16px]' }} />
                                            <Textarea placeholder='Коментарий курьеру, ориентир место доставки и тд.' classNames={{
                                                input: 'h-[150px] rounded-[16px]'
                                            }} />
                                            <Alert>
                                                После подтверждения заявки вам доставят товар в ближайшие времени. Пожалуйста, ждите звонка.
                                            </Alert>
                                        </Flex>
                                    </Paper>
                                            ...
                                            {sortedProducts?.map((store: any, index) =>
                                                <Flex direction={'column'} key={index}>
                                                    <Box>
                                                        ...
                                                        <Text className='text-[12px] text-[rgba(0,0,0,0.3)]'>Продавец: {store?.storeName}</Text>
                                                    </Box>
                ...
                {cart?.products.length >= 1 && <div className='h-full lg:w-1/3 w-full pt-10'>
                    <div className=' bg-[rgb(247,248,249)] p-5 rounded-md'>
                        ...
                        <br />
                        <Button bg={'green'} className='w-full text-[18px] h-[50px]'>Оформить</Button>
                    </div>
                </div>}
            </div>
        </Container>
